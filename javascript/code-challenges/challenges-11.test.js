...
const count = (target, input) => {
  // Solution code here...
  let value = 0;
  input.map(num => num.map(y => y === target && value++));
  return value;
};
...
const totalSum = (input) => {
  // Solution code here...
  let value = 0;
  input.map(num => num.map(y => value += y));
  return value;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  return input.map(element => element.filter(y => ((typeof y === 'number') && (y % 5 === 0)))
    .map(x => Math.pow(2, x)));
};
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  return data.filter(y => y.gender === 'female' || y.gender === 'male').map(x => x.name).join(' and ');
